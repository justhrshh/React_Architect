...
import { getCalleeName, getCalleeProperty, extractStringOrTemplate } from "../../../parser/astUtils.js";
import { createExtractionResult } from "../../../types/schemas.js";

export function extract(ast, context = {}) {
  const apiData = extractApi(ast, context.code || "");
  return createExtractionResult("api", apiData);
}
...
export function extractApi(ast, code) {
  const apiDetails = [];
  if (!ast || !ast.program) return apiDetails;
