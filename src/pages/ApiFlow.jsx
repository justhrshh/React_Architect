...
import {
  ...
  ChevronRight, GitBranch, ArrowLeft, FileCode, Radio,
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                {reduxFiles.filter(f => f.toLowerCase().includes("api") || f.toLowerCase().includes("service") || f.toLowerCase().includes("fetch") || f.toLowerCase().includes("endpoints")).map((f) => {
                  ...
                  const matchingNode = apiNodes.find(n => n.file === f);
                  return (
                    <div
                      key={f}
                      onClick={() => {
              ...
              <div className="mt-2 pt-4 border-t border-neutral-200/80">
                ...
                <div className="flex flex-col gap-2.5 max-h-80 overflow-y-auto pr-1">
                  {validation.errors.map((err, i) => {
                    const matchingNode = err.file ? apiNodes.find(n => n.file === err.file) : null;
                    return (
                      <div
                        key={`err-${i}`}
                        onClick={() => {
                  ...
                  {validation.warnings.map((warn, i) => {
                    const matchingNode = warn.file ? apiNodes.find(n => n.file === warn.file) : null;
                    return (
                      <div
                        key={`warn-${i}`}
                        onClick={() => {
