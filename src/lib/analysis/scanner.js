...
/**
 * Recursively scans a FileSystemDirectoryHandle for supported JS/TS files.
 *
...
/**
 * Scans a ZIP File object for supported JS/TS files.
 *
 * @param {File} zipFile
 * @returns {Promise<Array<{name: string, path: string, content: string}>>}
 */
  ...
  try {
    ...
    for (const [relativePath, zipEntry] of Object.entries(zip.files)) {
      ...
      if (supportedExtensions.has(ext)) {
        ...
        // Clean paths by removing top level directory wrapping if present in zip
        files.push({
