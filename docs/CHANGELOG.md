...
* **Sprint 13.4 Persistent Workspace Sessions**: Integrated initialization status tracking in `hubSlice.js`. Bypassed the workspace core boot sequence and loading animations by checking initialization status on mount, preventing replaying the animations when switching between studios.
* **Sprint 13.3 Project-Scoped Conversations**: Scoped AI conversation history dynamically based on the active project identifier fetched from the Redux store. Stored histories in `localStorage` namespace to automatically preserve session context across studio changes, routing, and browser page refreshes.
* **Sprint 13.2 Integrated Code Explorer**: Added an inline Code Explorer panel inside the AI Studio (`Investigation.jsx`) that slides in when an entity reference is clicked, preventing automatic navigation. Updates active Redux node selection to contextually scope subsequent follow-up queries. Shows architecture specs, LOC, API endpoints, props, hooks, imports, exports, and relationships, with a secondary user-initiated `Open in Architecture Studio` trigger.
* **Fixed Object as React Child Bug**: Resolved a runtime render crash when component properties (props, hooks, imports) contain structured objects rather than simple strings, mapping them to formatted text tags safely.
* **Origin-Based Back Navigation**: Added origin tracking in `Architecture.jsx` utilizing react-router's location state. Pressing `Esc` or clicking the top-left button (dynamically relabeled as `"Back (Esc)"`) routes the user back to their correct previous context (AI studio, Workspace, or Hub).
* **Sprint 13.2 Architecture Studio Interactivity**: Built a navigation bridge matching referenced entity names against the active Knowledge Graph, rendering them as interactive clickable elements in `MarkdownRenderer.jsx`. Clicking an entity transitions to the `/architecture` route, sets the active tab to `flow`, opens the inspector drawer, and auto-focuses/centers the camera on the target node.
* **Fixed Single-Item Interactivity Bug**: Corrected mapping resolution inside `parseInline` so single markdown code blocks (e.g. backticked items) are processed before array collapsing, restoring click navigation for all code block targets.
